...
static void
update_status_callback(xmlNode * msg, int call_id, int rc, xmlNode * output, void *user_data)
{
...
update_status_tag(const char *uname, const char *start_state)
{
    if (fsa_cib_conn && uname) {
        ...
        crm_info("Erasing lrm for %s", uname);
        rc = fsa_cib_conn->cmds->delete(fsa_cib_conn, xpath, NULL, cib_quorum_override | cib_xpath | cib_scope_local);
        fsa_register_cib_callback(rc, FALSE, data, update_status_callback);
    }
}
